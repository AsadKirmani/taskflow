...
@Component({
  ...
  template: `
    <div
      class="w-[300px] sm:w-[320px] flex flex-col h-full max-h-full rounded-xl bg-base-200/50 border border-base-300/5"
    >
      <div class="p-3 shrink-0 flex items-center justify-between group cursor-grab">
        <div class="flex items-center gap-2">
          ...
          <span
            class="text-xs font-medium text-base-content/50 bg-base-300 px-2 py-0.5 rounded-full"
          >
            {{ count() }}
          </span>
        </div>
...
export class UiBoardColumnComponent {
  ...
  statusColor = input<string>('bg-base-content/20');
}
