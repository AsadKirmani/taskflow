...
@Component({
  ...
  template: `
    <div
      [class]="
        'bg-base-100 rounded-box border border-base-300/10 overflow-hidden transition-all duration-200 ' +
        (interactive()
          ? 'hover:shadow-md hover:border-base-300 cursor-pointer active:scale-[0.99]'
          : 'shadow-sm')
      "
    >
      @if (hasHeader()) {
