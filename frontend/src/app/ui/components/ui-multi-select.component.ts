    ...
    <div class="relative w-full text-left" (click)="$event.stopPropagation()">
      ...
      >
        ...
        @for (option of selectedOptions(); track option.value) {
          <span class="inline-flex items-center gap-1 px-2 py-1 bg-base-200 border border-base-300/10 rounded text-xs font-medium text-base-content">
            {{ option.label }}
      ...
      @if (isOpen()) {
        <div class="absolute z-50 mt-1 w-full bg-base-100 border border-base-300/10 rounded-box shadow-xl max-h-60 overflow-y-auto p-1 animate-in fade-in zoom-in-95 duration-100">
