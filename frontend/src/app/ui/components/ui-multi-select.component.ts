...
@Component({
  ...
  template: `
    <div class="relative w-full text-left" (click)="$event.stopPropagation()">
      ...
      <div
        (click)="toggleOpen()"
        class="min-h-10 w-full p-1.5 bg-base-100 border border-base-300 rounded-btn cursor-pointer transition-colors hover:border-base-300/30 focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary flex flex-wrap gap-1 items-center"
      >
        ...
        @for (option of selectedOptions(); track option.value) {
          <span
            class="inline-flex items-center gap-1 px-2 py-1 bg-base-200 border border-base-300/10 rounded text-xs font-medium text-base-content"
          >
            {{ option.label }}
      ...
      @if (isOpen()) {
        <div
          class="absolute z-50 mt-1 w-full bg-base-100 border border-base-300/10 rounded-box shadow-xl max-h-60 overflow-y-auto p-1 animate-in fade-in zoom-in-95 duration-100"
        >
          @for (option of options(); track option.value) {
            <label
              class="flex items-center gap-2.5 px-3 py-2 hover:bg-base-200 rounded-md cursor-pointer transition-colors"
            >
              <input
                type="checkbox"
