  ...
  <div class="flex-1">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
      @for (file of attachments(); track file.url) {
        <!-- 🚀 Anchor tag use karo aur href mein file.url daalo -->
        <a
          [href]="file.url"
          target="_blank"
        ...
        >
          <div class="w-12 h-12 bg-base-200 rounded flex items-center justify-center flex-shrink-0 text-base-content/70 group-hover:text-base-content transition-colors overflow-hidden">
            <!-- Agar image hai toh preview dikhao, warna icon -->
          ...
          <div class="min-w-0 flex-1">
            <p class="text-sm font-semibold text-base-content truncate m-0 group-hover:underline group-hover:text-primary">
              {{ file.filename }}
            </p>
            <p class="text-xs text-base-content/70 m-0 mt-0.5 hover:text-primary">Click to view</p>
          </div>
        </a>
      }
    </div>
