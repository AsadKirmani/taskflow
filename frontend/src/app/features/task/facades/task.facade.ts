...
>([]);
  ...
  toggleTaskMember(userId: string) {
    ...
    const memberMap = new Map(this.boardMembers().map((member) => [member.id, member]))
    return (this.currentTask()?.assigneeIds || [])
      .map((id) => memberMap.get(id))
      .filter((member): member is User => !!member);
  });
  updateTaskDate(dates: { startDate: Date | null; dueDate: Date | null; startTime?: string | null; endTime?: string | null }) {
  const task = this.currentTask();
  if (!task) return;
