  ...
  template: `
   <h2 class="text-lg font-semibold mb-4">Security Settings</h2>
                <form [formGroup]="securityForm" (ngSubmit)="onSubmit()" class="space-y-4 w-full max-w-md">
                  ...
                  <button type="submit" [disabled]="securityForm.invalid || store.isSaving()" class="bg-error text-error-content px-5 font-medium py-2.5 rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center gap-2">
