...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-stone-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-2xl shadow-sm border border-stone-200 p-8 max-w-md w-full text-center">
        ...
        @else if (status() === 'error') {
          ...
          <button (click)="goToHome()" class="w-full bg-stone-900 text-white font-semibold py-2.5 rounded-xl hover:bg-black transition-colors">
            Go to Dashboard
          </button>
        }
...
export class AcceptInviteComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.workspaceApi.acceptWorkspaceInvite(token).subscribe({
      ...
      error: (err) => {
        this.status.set('error');
        this.errorMessage.set(err.error?.error || 'Failed to accept the invitation. It might have expired.');
